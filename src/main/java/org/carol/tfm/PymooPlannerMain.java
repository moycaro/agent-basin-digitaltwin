...
import org.carol.tfm.agents.planner.PymooPlanner;
...
public class PymooPlannerMain {

    public static void main(String[] args) throws Exception {
        LoggerContext context = (LoggerContext)
                LogManager.getContext(false);
        context.setConfigLocation(PymooPlannerMain.class
                .getResource("log4j2.configurationFile").toURI());

        //new Main().createAndShowGUI();
        PymooPlannerMain runner = new PymooPlannerMain();
    }
    ...
    public PymooPlannerMain() {
      ...
      //  this.agentTestPanel = new PilotoPanel();
        ...
        try {
            ...
            PymooPlanner pymooPlanner = new PymooPlanner( this.reservoir_agents, this.basicManageDamCapability, timeStepManagementCapability.getBeliefBase() );
            AgentController acBasicPlanner = ((AgentContainer) controller).acceptNewAgent("Planner::Pymoo", pymooPlanner );
            ...
            this.basicManageDamCapability.values().stream().forEach(believeBase -> {
                    believeBase.addBeliefListener(pymooPlanner);
